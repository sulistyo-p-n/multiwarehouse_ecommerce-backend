...
import com.multiwarehouse.app.inventory.service.messaging.mapper.WarehouseMessagingDataMapper;
...
public class WarehouseChangeKafkaListener implements KafkaConsumer<WarehouseChangeAvroModel> {
    ...
    @Override
    @KafkaListener(id = "${kafka-consumer-config.warehouse-change-consumer-group-id}", topics = "${inventory-service.warehouse-change-topic-name}")
    public void receive(
